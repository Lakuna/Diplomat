...
def execute(query):
	...
	for i in range(iterations):
		...
		# Update progress bar.
		print_progress_bar(i + 1, iterations, prefix='Simulation:', suffix='Complete', length=50)

		# End of game.

	return_string = 'Iteration complete\nGames played: ' + str(iterations) + '\nTotal turns: ' + str(total_turns)
	if selected_nation != None:
		return_string += '\nSelected nation [' + selected_nation + '] best next actions:'
		for action in watched_actions:
			return_string += '\n' + action + '\t(used in ' + "{0:.1f}".format(100 * (watched_actions[action] / float(nation_wins[selected_nation]))) + '% of wins)'
	return_string += '\n\nStatistics by nation:'
	for nation in nation_wins:
		return_string += '\n' + nation + '\tAverage placement: ' + '{0:.2f}'.format(rank_total[nation] / float(iterations)) + '\tAverage turns until win: ' + '{0:.1f}'.format(nation_win_turns[nation] / float(nation_wins[nation])) + '\tFastest turns until win: ' + str(fastest_win[nation]) + '\tWinrate: ' + "{0:.1f}".format(100 * (nation_wins[nation] / float(iterations))) + '%'
